...
def create_mockup(index, icon_file, title, subtitle):
    ...
    # 5. Tablet Scaling
    # Tablet has wider screen relative to bezel
    target_w = 900
    ...
    screenshot = add_rounded_corners(screenshot, radius=30) # Sharper corners for tablet
    
    # 6. Tablet Frame (Thick uniform bezel)
    bezel_padding = 25 # Much thicker for 7-inch tablet look
    ...
    draw.rounded_rectangle(
        ...
        radius=50, fill=None, outline=(15, 15, 18), width=bezel_padding
    )
    # Highlight Edge
    draw.rounded_rectangle(
        [phone_x, phone_y, phone_x + bezel_w, phone_y + bezel_h],
        radius=50, fill=None, outline=(50, 52, 55), width=3
    )
    
    # Camera (at the very top of bezel)
    cam_x = CANVAS_SIZE[0] // 2
    cam_y = phone_y + (bezel_padding // 2)
    draw.ellipse([cam_x - 8, cam_y - 8, cam_x + 8, cam_y + 8], fill=(0, 0, 0))
    
    # 8. Save as PNG
    output_path = os.path.join(OUTPUT_DIR, f"Play Store 7 inch {index+1}.png")
    canvas.save(output_path, "PNG")
    print(f"Created Tablet Mockup: {output_path}")
