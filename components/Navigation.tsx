    ...
    <div className="fixed bottom-0 left-0 right-0 mx-auto max-w-md bg-white/90 backdrop-blur-xl rounded-t-[32px] border-t border-white/20 pb-safe pt-3 px-4 shadow-[0_-8px_32px_rgba(0,0,0,0.12)] z-50">
      <div className="grid grid-cols-5 items-end relative pb-2">
        ...
        {tabs.map((tab) => {
          ...
          if (tab.isAction) {
            return (
              <div key={tab.id} className="flex justify-center relative -top-8">
                <button
                  onClick={onAdd}
                ...
                >
                  <motion.div
                    ...
                    className="bg-primary text-white p-4 rounded-full shadow-xl shadow-indigo-500/40 ring-4 ring-white group-hover:shadow-indigo-500/60 transition-shadow"
                  >
          ...
          return (
            <motion.button
              ...
              className={`flex flex-col items-center justify-center pb-2 transition-all duration-300 relative z-10 ${isActive ? 'text-primary' : 'text-gray-400 hover:text-indigo-500'}`}
            >
              <div className={`absolute inset-0 bg-indigo-50/50 rounded-2xl z-[-1] transition-opacity duration-300 ${isActive ? 'opacity-100 scale-100' : 'opacity-0 scale-75 group-hover:opacity-50'}`} />

              <div className="relative p-2">
                <Icon size={28} strokeWidth={isActive ? 2.5 : 2} />
              </div>
              <span className={`text-[10px] font-medium transition-all ${isActive ? 'text-primary font-bold translate-y-0' : 'text-gray-400 translate-y-0.5'}`}>{tab.label}</span>
            </motion.button>
          );
        })}
      </div>
